...
class FiveDayBreakoutStrategy(BaseStrategy):
    ...
    def backtest(self, df, costs):
        ...
        df["prev_high"] = (df.groupby("code")["high"]
                           .shift(1).rolling(self.lookback, min_periods=self.lookback).max())
        df["prev_vol_mean"] = (df.groupby("code")["volume"]
                               .shift(1).rolling(self.lookback, min_periods=self.lookback).mean())
        df["signal"] = (
